...
/* ===================================================================================
   ...
   - Primary source: sessionStorage.getItem('sessionUser')
   - Fallback: localStorage.getItem('auth') / localStorage.getItem('sessionUser') / older shapes
   - Normalizes to: role, userid, teacherid, stu_curr_semester, stu_section
=================================================================================== */
function getSessionUser() {
  ...
  // Prefer the sessionStorage shape from your Login:
  // {
  //   userid, user_role, role_description, roles,
  //   stuuserid, student_semester, student_section,
  //   teacher_userid, teacher_id, login_time
  // }
  let src = ss || lsAuth || lsSess || {};

  // Normalize role (handles "user_teacher", "USR_TCHR", etc.)
  const role = (() => {
  ...
  const teacherid =
    ...
    src.teacherID ||
    src.teacherId ||
    src.teacher_userid || // last resort if only user id stored as "teacher_userid"
    "";

  const stu_sem =
    src.student_semester ||
    src.stu_curr_semester ||
    src.stuSemester ||
    src.semester ||
    "";
...
// Role flags (robust even if someone changes the normalizer)
...
// ---------- helpers bound to offerings ----------
function getOffering(offeringId, offerings) {
  if (!offeringId) return null;
  return offerings.find((o) => String(o.offerid) === String(offeringId)) || null;
}
 ...
 *  - Compares routine's possible teacher-id fields against session teacherId (and userId as fallback)
...
function routineBelongsToSessionTeacher(r, userId, teacherId) {
  ...
  // Common fields that may hold the teacher identifier
  const candidates = [
    r.drteacherid,              // e.g., 90
    r.drclassteacherid,
    r.classteacherid,
    r.teacherid,
    r.teacher_id,
    ...
    r.teacher?.id,
    r.teacher_userid,
    r.drclassteacher_userid,
    r.class_teacher_userid,
    r.classteacher_userid,
  ].map(norm).filter(Boolean);

  // Prefer teacherId match; also allow userId match if that's what your data stores
...
export default function DailyRoutine() {
  ...
  // NEW: effective teacher id derived when missing from session
  const [effectiveTeacherId, setEffectiveTeacherId] = useState(
    String(sessTeacherId || "")
  );
  ...
  // ✅ EARLY ROLE/SESSION GUARDS (teacher needs user_id OR teacher_id; student needs semester+section)
  ...
  useEffect(() => {
    ...
    // If session already had one, just keep it
    if (sessTeacherId) {
      setEffectiveTeacherId(String(sessTeacherId));
      return;
    }

    // Derive from teachers list using the logged-in user id
    if (teachers && teachers.length && sessUserId) {
      const match = teachers.find(
        (t) =>
          String(
            t.teacheruserid ?? t.userid ?? t.user_id ?? t.userId
          ) === String(sessUserId)
      );
      ...
      if (match?.teacherid) {
        ...
        // Persist to sessionStorage so next page load has it
        try {
  ...
  // Handy console confirmation (as requested)
  useEffect(() => {
    if (isTeacher) {
  ...
  }, [isTeacher, effectiveTeacherId, sessTeacherId]);
  ...
  const getRoutineKey = (routine) =>
    [
      ...
      routine.drslot,
      routine.drsubjid,
      routine.drclassroomid,
      routine.stu_curr_semester,
      routine.stu_section,
      routine.acad_year,
      routine.drdate,
    ].join("_");
  ...
  // click on a table cell to prefill & open Add Routine
  const handleCellClick = (dayIdx, slotFloat) => {
    if (!isAdmin) return; // 🔒 only admin can add by clicking grid
  ...
  // sometimes we still need raw subject id from offering
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!isAdmin) return; // 🔒 only admin can submit add/update
    setLoading(true);
  ...
  const currentTeacher = useMemo(() => {
    ...
    // Prefer match by the effective teacher id; fall back to teacheruserid/userid
  ...
  // 🌟 Session-based visibility (first clamp by role, then apply UI filters)
  let sessionFilteredRoutines = routines;

  if (isTeacher && (effectiveTeacherId || sessUserId)) {
    // Teacher sees ONLY their own schedules (supports multiple possible routine fields)
  ...
  // Only show routines at their START time in the grid (no duplicates)
    ...
    const occupiedTeacherIds = routines
      .filter(
        (r) =>
          ...
          to24Hour(r.drto) > fromTime &&
          (!editingRoutine ||
            getRoutineKey(r) !== getRoutineKey({ ...form, ...editingRoutine }))
      )
  ...
  return (
    ...
    >
      ...
      >
        ...
        {isTeacher && (
          <>
            ...
            {/* use effectiveTeacherId instead of raw session value */}
      ...
      {/* Admin-only filters (teachers and students don’t need these) */}
      {isAdmin && (
        <div
          style={{
            display: "flex",
                  ...
                  >
                    ...
                    >
                      {getRoutinesAt(dayIdx, slot).map((routine) => {
                        ...
                        // ✅ use master lists to show descriptions, not IDs
