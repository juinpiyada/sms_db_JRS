...
export default function UserRole() {
  ...
  const fetchMasterRoles = async () => {
    try {
      ...
      setMasterRoles(Array.isArray(res.data) ? res.data : []);
    } catch {
      setMasterRoles([]);
    }
  };
  ...
  const handleRoleDescChange = (e) => {
    ...
    setFormData({
      ...
      userrolesid: found ? found.role_id : ''
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        await axios.put(`${config.USER_ROLE_ROUTE}/${userid}/${userrolesid}`, { userroledesc });
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`${config.USER_ROLE_ROUTE}/${deleteInfo.userid}/${deleteInfo.userrolesid}`);
